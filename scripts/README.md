...
| `cli_sweep.py` | CLI 全子命令端到端验收（18 个用例：查询/批量/对比/覆盖参数/错误路径），报告写入 `logs/_cli_sweep.txt` | `python scripts/cli_sweep.py` | ✅ **可复用**：改动 CLI 后回归 |
...
- `cli_sweep.py` 依赖真实网络（首个城市会用缓存），退出码非 0 表示有用例异常；详细输出见 `logs/_cli_sweep.txt`。
